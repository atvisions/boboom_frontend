...
export function TradesAndHolders({
  tokenAddress,
  token,
  okbPrice,
}: TradesAndHoldersProps) {
  const [activeTab, setActiveTab] = useState<"overview" | "trades" | "holders">(
    "overview"
  );
          ...
          setTransactions((prev) => {
            ...
            const exists = prev.some(
              (tx) =>
                ...
                tx.hash === mappedTransaction.hash
            );
            if (!exists) {
  ...
  useEffect(() => {
    ...
    if (activeTab === "trades") {
      ...
      holdersConnectionId = websocketService.connect(
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-gray-400 text-sm max-md:hidden">
          {activeTab === "overview" ? (
            ...
            <span>
              Showing {holders.length} of {totalHolders} holders
            </span>
          )}
        </div>
      </div>
          ...
          <div className="bg-gradient-to-br from-[#151515] to-[#1a1a1a] border border-[#232323] rounded-2xl p-6">
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <button
                  onClick={() =>
                    activeTab === "trades" ? loadTransactions() : loadHolders()
                  }
          ...
          {activeTab === "trades" && (
            <div className="space-y-3">
              {transactions.map((tx, index) => (
                <div
                  key={tx.id || index}
                  className="p-3 bg-[#1a1a1a] rounded-lg border border-[#232323] hover:border-[#D7FE11]/30 transition-all duration-200"
                >
                  {/* 左侧：交易类型和用户 */}
                  <div className="flex items-center justify-between space-x-3">
                    ...
                    <div className="flex justify-between flex-1 items-start gap-2 md:items-center flex-col md:flex-row">
                      {/* 用户地址和交易hash */}
                      <div className="flex flex-col space-y-1">
                        ...
                        {tx.transaction_hash && (
                          <div className="flex items-center space-x-2 text-xs text-gray-400">
                            ...
                            >
                              {tx.transaction_hash.startsWith("0x")
                                ? tx.transaction_hash.slice(2, 10)
                      ...
                      <div className="md:text-right space-y-1 w-full md:w-auto">
                        ...
                        <div className="flex items-center space-x-4 text-sm">
                          <span className="text-white font-medium">
                            {tx.type === "CREATE"
                              ? "Created"
                              : `${tx.type === "BUY" ? "+" : "-"}${parseFloat(
                                  tx.amount || tx.token_amount || "0"
                                ).toFixed(6)}`}
                          </span>
