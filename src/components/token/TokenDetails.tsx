        ...
        try {
          ...
          const creatorAddress =
            typeof token.creator === "string"
              ? token.creator.toLowerCase() === token.address.toLowerCase()
                ? ""
                : token.creator
          ...
          if (token.creator && typeof token.creator === "object") {
            ...
            setCreator({
              ...
              username: `${token.creator.slice(0, 6)}...${token.creator.slice(
                -4
              )}`,
              avatar_url: "👤",
            });
          ...
          <div className="flex justify-between">
            ...
            <div className="flex items-center space-x-2 flex-shrink-0">
              ...
              >
                <Star
                  className={`h-4 w-4 ${
                    isFavoriteLoading ? "animate-spin" : ""
                  }`}
                />
              </button>
            </div>
              ...
              >
                ...
                <span className="text-gray-300 text-sm font-medium group-hover:text-white transition-colors">
                  {creator?.username ||
                    ...
                    (typeof token.creator === "string"
                      ? `${token.creator.slice(0, 6)}...${token.creator.slice(
                          -4
                        )}`
                      : token.creator?.address
                      ? `${token.creator.address.slice(
                          0,
                          6
                        )}...${token.creator.address.slice(-4)}`
                      : "Unknown")}
                </span>
