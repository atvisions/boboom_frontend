...
export function TokenGrid() {
        ...
        if (Array.isArray(tokenList)) {
          ...
          if (selectedSort === "graduated") {
            tokenList = tokenList.filter((token: any) => {
              const isGraduated =
                token.phase === "GRADUATED" || token.has_graduated === true;
              if (!isGraduated) {
              }
              return isGraduated;
            });
          ...
          const processedTokens = tokenList.map((token: any) => ({
            ...
            createdAt:
              token.created_at || token.createdAt || new Date().toISOString(),
            ...
            transactionCount:
              token.transaction_count || token.transactionCount || 0,
          ...
          if (response.success) {
            ...
            const processedTokens = rawTokens.map((token: any) => {
              const processed = {
                ...
                // 映射下划线字段名到驼峰格式
                graduationProgress: parseFloat(
                  token.graduation_progress || token.graduationProgress || "0"
                ),
                volume24h: token.volume_24h || token.volume24h || "0",
                marketCap: token.market_cap || token.marketCap || "0",
                currentPrice: token.current_price || token.currentPrice || "0",
  ...
  // 防抖的收藏状态加载
  const [favoriteTimeout, setFavoriteTimeout] = useState<NodeJS.Timeout | null>(
    null
  );
  ...
  const parseNumberInput = (input: string): number => {
    ...
    const numPart =
      parseFloat(cleanInput.slice(0, -1)) || parseFloat(cleanInput);
  ...
  const applyFiltersWithCustomFilters = (customFilters: any) => {
    if (originalTokens.length > 0) {
      ...
      // 检查是否有活跃的筛选条件
      const hasCustomActiveFilters = !!(
        customFilters.mcapMin ||
        customFilters.mcapMax ||
        customFilters.volumeMin ||
        customFilters.volumeMax
      );
  ...
  return (
    <div className="px-6 pb-8">
      {/* 标题和筛选器 */}
      <div className="flex justify-between mb-8 md:flex-row flex-col md:items-center items-start gap-4">
        <div className="flex items-center space-x-3">
          {/* <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl flex items-center justify-center">
            <TrendingUp className="h-5 w-5 text-white" />
          </div> */}
          <div>
            <h2 className="text-3xl font-bold text-white font-hubot-sans">
              Live Tokens
            </h2>
          </div>
        </div>
            ...
            {showFilter && (
              <>
                ...
                >
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleFilterChange("mcapMin", e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleFilterChange("mcapMax", e.target.value)
                            }
                            className="w-full px-3 py-2 bg-[#232323] border border-gray-600 rounded-lg text-white text-sm focus:outline-none focus:border-[#D7FE11]"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleFilterChange("volumeMin", e.target.value)
                            }
                            className="w-full px-3 py-2 bg-[#232323] border border-gray-600 rounded-lg text-white text-sm focus:outline-none focus:border-[#D7FE11]"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleFilterChange("volumeMax", e.target.value)
                            }
                            className="w-full px-3 py-2 bg-[#232323] border border-gray-600 rounded-lg text-white text-sm focus:outline-none focus:border-[#D7FE11]"
                          />
                        </div>
                  ...
                  {/* 操作按钮 - 移动端固定底部 */}
                  <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center mt-6 gap-3 md:gap-0">
                    ...
                    <Button
                      ...
                      className="bg-[#D7FE11] text-black hover:bg-[#5BC500] px-6 py-3 font-medium"
                    >
                      Apply Filters
                    </Button>
                  </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                {filters.mcapMin && (
                  ...
                  >
                    ...
                    <span className="text-[#D7FE11]/70 hover:text-[#D7FE11]">
                      ×
                    </span>
                  </button>
                )}
      ...
      {loading && tokens.length === 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
          {[...Array(12)].map((_, index) => (
            <div
              key={index}
              className="group relative bg-gradient-to-br from-[#151515] to-[#1a1a1a] border border-[#232323] rounded-2xl p-6 animate-pulse"
            >
              {/* 收藏按钮骨架 - 右上角 */}
                  ...
                  {token.imageUrl ? (
                    <Image
                      ...
                      onError={(e) => {
                        ...
                        if (parent) {
                          parent.innerHTML = `<span class="text-2xl font-bold text-white">${token.symbol.slice(
                            0,
                            2
                          )}</span>`;
                        }
                        ...
                        <div className="w-5 h-5 rounded-full bg-gradient-to-br from-[#D7FE11]/20 to-[#5BC000]/20 flex items-center justify-center overflow-hidden">
                          {(() => {
                            ...
                            if (
                              ...
                              if (
                                creatorInfo.avatar_url.startsWith("/media/")
                              ) {
                                return (
                                  <Image
                                    src={`${
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[#D7FE11] font-bold text-sm">
